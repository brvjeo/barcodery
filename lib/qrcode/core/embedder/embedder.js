...
export function embed(matrix) {
    ...
    addSideFields(matrix);
    matrix.matrix[5 + 7 - 1][5 + 7 - 1] = 0;
}
function addSideFields(matrix){
    ...
    for(let i = 0;i < 5;i++){
        matrix.matrix.unshift(new Array(matrix.width + 10).fill(0));
        matrix.matrix.push(new Array(matrix.width + 10).fill(0));
    }
    matrix.width = matrix.height = matrix.width + 10;
}
